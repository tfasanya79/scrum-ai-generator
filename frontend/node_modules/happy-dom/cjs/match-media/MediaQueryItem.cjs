...
class MediaQueryItem {
    ...
    matchesMediaType(mediaType) {
        ...
        return mediaType === new WindowBrowserContext_js_1.default(this.window).getSettings()?.device.mediaType;
    }
    /**
     ...
     */
    matchesRule(rule) {
        const settings = new WindowBrowserContext_js_1.default(this.window).getSettings();
        if (!settings) {
            return false;
        }
        if (!rule.value) {
     ...
     */
    toPixels(value) {
        if (!new WindowBrowserContext_js_1.default(this.window).getSettings()?.disableComputedStyleRendering &&
            value.endsWith('em')) {
            this.rootFontSize =
                this.rootFontSize ||
